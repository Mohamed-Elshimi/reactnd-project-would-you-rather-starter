	...
	render() {
		...
		return (
			<Fragment>
				<Header as="h3">
					Results:
					<Header.Subheader style={{ fontWeight: 'bold' }}>Would you Rather ...</Header.Subheader>
				</Header>
				<Segment color={option1.color} style={{ backgroundColor: `${option1.bgColor}` }}>
					{userVote === 'optionOne' && <YourVoteLabel />}
					<p style={{ fontWeight: 'bold' }}>{question.optionOne.text}</p>
					<Progress percent={(optionOneVotes / votesTotal * 100).toFixed(2)} progress color={option1.color}>
				...
				<Segment color={option2.color} style={{ backgroundColor: `${option2.bgColor}` }}>
					...
					<Progress percent={(optionTwoVotes / votesTotal * 100).toFixed(2)} progress color={option2.color}>
